view: by_card_underlying_symbol_reg_emax_seq_rolling {
  derived_table: {
    ...
    sql:
         ...
         ,  first_trans AS (SELECT min(trans_date) FROM ${dist_day_sym_brand_cardtype_emax_currency.SQL_TABLE_NAME})
         ,  oldest_trans AS (SELECT brand_id, min(trans_date) as min_date FROM ${dist_day_sym_brand_cardtype_emax_currency.SQL_TABLE_NAME} group by brand_id)
    ...
    , current_period_sales AS (  select
                                      ...
                                      left join ${dist_day_sym_brand_cardtype_emax_currency.SQL_TABLE_NAME} p
    ...
    #                 and p.trans_date between sb.start_date and sb.end_date
         ...
         ,  symbol_period_table AS (select
                                     ...
                                     #select
                                     #    s.symbol
                                     #    , c.calendar_half as period --calendar_half needs to be added to calendar table
                                     #    , min(c.date) as period_start_dt
                                     #    , max(c.date) as period_end_dt
                                     #from (SELECT distinct symbol FROM ${ground_truth_brand.SQL_TABLE_NAME}) s
                                     #cross join ${calendar.SQL_TABLE_NAME} c

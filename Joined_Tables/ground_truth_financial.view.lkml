view: ground_truth_financial {
  derived_table: {
    sql:

     SELECT

      sd.symbol_id, sd.symbol, sd.company_name, sd.ric, sd.isin, sd.country_hq, sd.country_exchange, sd.ownership_status,

      CASE WHEN sd.ownership_status = "Public" THEN 1 ELSE 0 END as public_flag,

      ss.segment_id, ss.segment_name, ss.primary_flag,

      sfm.financial_id, sfm.fundamental, sfm.fundamental_shortcoming, sfm.suggested_remedy,

      fmd.reported_metric_summary, fmd.reported_metric_attrib1, fmd.reported_metric_attrib2, fmd.reported_metric_attrib3, fmd.reported_metric_type, fmd.reported_metric_currency, fmd.financial_type,

      smp.period_type, smp.period, smp.period_start_dt, smp.period_end_dt, smp.reported_metric, smp.est_date_flag,

      cfm.consensus_id, cfm.external_identifier, cfm.consensus_source, cfm.consensus_note, cfm.metric_unit, cfm.metric_currency,

      cp.period as consensus_period, cp.metric as consensus_metric
      ...
      INNER JOIN `ce-cloud-services.ce_transact_uk_ground_truth.segment_financial_metric` sfm
